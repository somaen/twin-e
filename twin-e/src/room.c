...
void
LBA_engine::loadRoomScene (int sceneNumber)
{
  unsigned char *temp;
  short int temp3;
//      short int temp4;
...
// numActorInRoom=roomMusic;
  ...
  while (currentActor < numActorInRoom)
	...
	{
	  loadDataFileToPtr ("file3d.hqr", modelNumber,
			     &actors[currentActor].bodyPtr);
	}
...
void
LBA_engine::changeRoomSub1 (int arg_0)
{
  int reinitVar1Copy;
  int reinitVar2Copy;
// int i;
...
void
LBA_engine::loadVariousGFX (void)
{
...
int
LBA_engine::loadGridAndBll (short int roomNumber)
{
  int gridSize;
  int bllSize;
  int brickDataSize;
  ...
  size =
    processBuffer2Buffer ((unsigned int *) bufferBrick,
			  (unsigned int *) bufferBrick2);
...
/*	dumpFile("cube",(char*)bufCube,204800);
	dumpFile("grid",(char*)currentGrid,10000);
  ...
  do
      ...
      do
	{
	  addCubeEntry (currentGrid +
			*(unsigned short int *) (currentGrid +
						 2 * (i + ptr2)),
			bufCube + ptr1);
	  ptr1 += 50;
	}
...
void
LBA_engine::addCubeEntry (unsigned char *gridEntry, unsigned char *dest)
{
...
void
LBA_engine::changeTwinsenComp (int newComportement)
{
  int temp;
...
void
LBA_engine::memoryBufferProcess (unsigned char *ptr, int size)
{
  int temp;
...
int
LBA_engine::processBuffer2Buffer (unsigned int *buffer1,
				  unsigned int *destBuffer)
{
...
int
LBA_engine::processBuffer2BufferSub (int var, unsigned int *buffer,
				     unsigned int *ptr)
{
...
void
LBA_engine::changeRoom1Sub1 (hqr_entry * ptr)
{
...
void
LBA_engine::changeRoom1Sub2 (void)
{
  byte *temp;
...
int
LBA_engine::loadBrk (int gridSize)
{
  int firstBrick;
  int lastBrick;
  int currentBrick;
...
void
LBA_engine::loadBrkSub1 (unsigned char *ptr, int size)
{
  int i;
