  ...
  do
    {
      ...
      scriptPtr =
	(char *) (lactor->positionInMoveScript + lactor->moveScript);
	...
	case 13:
	  ...
	  break;
	case 14:
	  fullRedrawS3 (*(short int *) scriptPtr, 0x1000, 1, lactor->X,
			lactor->Z, lactor->Y);
	  lactor->positionInMoveScript += 2;
	  break;
	case 15:
	  ...
	  if (lactor->field_60 & 0x400)
	    {
	      ...
	      lactor->angle =
		calcAngleToward (lactor->X, lactor->Y, destX, destY);
	      lactor->field_78 =
		calcAngleToward (lactor->Z, 0, destZ, moveActorVar1);
	...
	case 24:
	  ...
	  if (!temp)
		...
		default:
		  printf
		    ("Unsupported subopcode of actor move opcode 21-24!\n");
		  exit (1);
		}
	...
	case 28:
	  fullRedrawS3 (*(short int *) scriptPtr, 0x1000, 0, lactor->X,
			lactor->Z, lactor->Y);
	  lactor->positionInMoveScript += 2;
	  break;
	case 29:
...
void
LBA_engine::updateActorAngle (int angleFrom, int angleTo, int angleSpeed,
