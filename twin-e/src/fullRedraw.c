  ...
  for (arg_1A = 0; arg_1A < numActorInRoom; arg_1A++, arg_46++, arg_42++)	// process actors (and draw shadow if aplicable)
    {
      ...
      *(byte *) & lactor->field_62 &= 0xFFEF;	//recheck -> reinit the draw flags for the current objects
      ...
      if ((changeRoomVar9 == -1)
	  || (lactor->Z <=
	      *(short int *) ((currentGrid2) * 24 + sceneVarPtr + 8)))
	{
		  ...
		  if (((lactor->field_60 & 8) && fullRedrawVar3 > -112
		      ...
		      if (lactor->field_60 & 0x400)
			{
			  ...
			  if (lactor->field_60 & 8)
			    {
			      temp3 =
				lactor->lastX - cameraX + lactor->lastY -
				cameraY;
			    }
			}
		      else
...
/*			if(!(fullRedrawVar6[arg_1E].field_2&0x3FF))
	         ...
	         renderLeft=fullRedrawVar3-spriteWidth; // calculate center
	         renderRight=fullRedrawVar3+spriteWidth;

	         renderTop=fullRedrawVar4-spriteHeight;
	         renderBottom=fullRedrawVar4+spriteHeight;
  ...
  for (arg_1A = 0; arg_1A < numFlags; arg_1A++)	//affichage des flags
	...
	char stringTemp[256];
      ...
      fullRedrawS2S1 (flagData[arg_1A].x - cameraX,
		      flagData[arg_1A].z - cameraZ,
...
void
LBA_engine::fullRedrawSub3 (short int arg_0, short int arg_4, short int arg_8,
			    short int arg_C)
{
  int i;
  short int var1;
...
void
LBA_engine::fullRedrawSub1 (void)
{
  int i;
...
void
LBA_engine::fullRedrawSub5 (void)
{
  int i;
...
void
LBA_engine::addToRedrawBox (short int arg_0, short int arg_4, short int arg_8,
			    short int arg_C)
{
  int var1;
  int i = 0;
...
void
LBA_engine::zbuffer (int var1, int var2, int x, int z, int y)
{
  unsigned char *ptr;
...
unsigned char *
LBA_engine::zbufferSub1 (int var)
{
  return (currentBll + *(unsigned int *) (currentBll + 4 * var));
}

void
LBA_engine::zbufferSub2 (int x, int z, int y)
{
...
void
LBA_engine::drawSprite (int num, int var1, int var2,
...
void
LBA_engine::drawSprite2 (int num, int var1, int var2,
			 unsigned char *localBufferBrick)
{
  unsigned char *ptr;
  int top;
...
int
LBA_engine::fullRedrawS2S1 (int coX, int coZ, int coY)
{
  if (setSomethingVar4 != 1)
    ...
    {
      fullRedrawVar3 =
	(((coX - coY) * 8 + (coX - coY) * 16) >> 9) + setSomethingVar1;
      fullRedrawVar4 =
	((((coX + coY) * 4 + (coX + coY) * 8) +
...
int
LBA_engine::fullRedrawS3 (int param0, int param1, int param2, int param3,
...
void
LBA_engine::getSpriteSize (int arg_0, int *arg_4, int *arg_8, char *ptr)
{
  ptr += *(int *) (ptr + arg_0 * 4);
...
void
LBA_engine::refreshUpperBricks3d (int X, int Z, int Y)
{
  int refreshLeft;
  int refreshRight;
  int i;
...
void
LBA_engine::refreshUpperBricks (int X, int Z, int Y)
{
  int refreshLeft;
  int refreshRight;
  int i;
	  ...
	  for (i = 0; i < zbufferTab[refreshLeft]; i++)
	    {
		  ...
		  && currentZbufferData->z >= Z)
		{
		  if (currentZbufferData->x == Y
		      && currentZbufferData->y == X)
		    {
		      redrawBrick (currentZbufferData->spriteNum,
				   (refreshLeft * 24) - 24,
				   currentZbufferData->drawY, bufferBrick2,
...
void
LBA_engine::redrawBrick (int spriteNum, int x, int y, byte * localBufferBrick,
			 byte * buffer)
{
  unsigned char *ptr;
  ...
  ptr =
    localBufferBrick + *(unsigned int *) (localBufferBrick + spriteNum * 4);
...
void
LBA_engine::addToRedrawBoxMain (int left, int top, int right, int bottom)
{
  if (left < 0)
    left = 0;
  if (top < 0)
...
void
LBA_engine::sortRenderList (fullRedrawVar6Struct list, int listSize,
			    int param)
{
...
void
LBA_engine::fullRedrawSub11 (void)
{
  int i;
