...
void LBA_engine::drawMenuWin2(short int arg_0, short int arg_4)
{
	...
	{
	    drawSprite(0, remapComposante(arg_0 + 25, arg_0 + 325, 10, i), arg_4 + 58,
		       getHqrdataPtr(HQRPtrSpriteExtra, 41));
	}

    for (i = 0; i < numClover; i++)	// trefles
	{
	    drawSprite(0, remapComposante(arg_0 + 25, arg_0 + 325, 10, i) + 2, arg_4 + 60,
		       getHqrdataPtr(HQRPtrSpriteExtra, 7));
	}
