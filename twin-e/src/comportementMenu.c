...
LBA_engine::drawComportementEntry (int lcomportement, int arg, int arg2)
{
  ...
  if (draw3D1
      (currentAnimSate, (char *) currentAnim, (char *) menuCostumeIndex))
    {
  ...
  if (arg2 == 0)
    {
      drawBoxOutLine (box_left - 1, box_top - 1, box_right + 1,
		      box_bottom + 1);
    }
...
void
LBA_engine::drawMenuWin2 (short int arg_0, short int arg_4)
{
  int var_10;
  int var_4;
  int var_8;
  ...
  if (!vars[28] && vars[4])	// ca doit etre le mana
    ...
    {
      drawSprite (0, var_14, arg_4 + 36,
		  getHqrdataPtr (HQRPtrSpriteExtra, 5));
...
void
LBA_engine::drawMenuWin (short int var)
{
  drawBoxOutLine (100, 100, 550, 290);
  drawBoxInsideTrans (101, 101, 549, 289, 2);

  setAnimAtKeyFrame (winTab[0], getHqrdataPtr (HQRanims, TCOS[0]),
		     menuCostumeIndex);
  drawComportementEntry (0, var, 0);

  setAnimAtKeyFrame (winTab[1], getHqrdataPtr (HQRanims, TCOS[1]),
		     menuCostumeIndex);
  drawComportementEntry (1, var, 0);

  setAnimAtKeyFrame (winTab[2], getHqrdataPtr (HQRanims, TCOS[2]),
		     menuCostumeIndex);
  drawComportementEntry (2, var, 0);

  setAnimAtKeyFrame (winTab[3], getHqrdataPtr (HQRanims, TCOS[3]),
		     menuCostumeIndex);
...
void
LBA_engine::processComportementMenu (void)
{
