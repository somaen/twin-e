...
void debugger::addButton(int winIndex,int X,int Y,int width, int height,char* text,buttonType type)
{
...
buttonType debugger::findButton(int X,int Y)
{
	...
	for(i=0;i<numOfWindows;i++)
	{
		...
		{
			if(X>(windows[i].X+windows[i].buttons[j].X) && X<(windows[i].X+windows[i].buttons[j].X+windows[i].buttons[j].Width)
				&& Y>(windows[i].Y+windows[i].buttons[j].Y) && Y<(windows[i].Y+windows[i].buttons[j].Y+windows[i].buttons[j].Height))
			{
...
void debugger::drawAll()
{
	...
	for(i=0;i<numOfWindows;i++)
	{
		...
		for(j=0;j<windows[i].numOfButtons;j++)
		{
			fillArea2(X+windows[i].buttons[j].X,Y+windows[i].buttons[j].Y,windows[i].buttons[j].Width,windows[i].buttons[j].Height);
		...
		case 23:
			{
				...
				sprintf(buffer,"SET_TRACK %d (offset)",temp);
				addLine(buffer,script);
				break;
			}
		case 24:
			{
				...
				sprintf(buffer,"SET_TRACK %d (offset) for actor %d",temp2,temp1);
				addLine(buffer,script);
				break;
			}
		case 25:
