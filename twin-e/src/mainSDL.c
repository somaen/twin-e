...
OSystem::OSystem (int argc, char *argv[])	// that's the creator of the system dependent object used for the SDL port
{
  ...
  for (i = 0; i < 16; i++)
    {
      surfaceTable[i] =
	SDL_CreateRGBSurface (SDL_SWSURFACE, 640, 480, 32, rmask, gmask,
			      bmask, 0);
    }
...
void
OSystem::setColor (byte i, byte R, byte G, byte B)	// cette fonction est vraiment vraiment tres tres lente...
{
  sdl_colors[i].r = R;
  sdl_colors[i].g = G;
  sdl_colors[i].b = B;
...
void
OSystem::drawBufferToScreen (unsigned char *videoBuffer)
{
  SDL_BlitSurface (sdl_buffer, NULL, sdl_screen, NULL);
...
void
OSystem::refresh (unsigned char *videoBuffer, int left, int top, int right,
		  int bottom)
{
  SDL_Rect rectangle;
...
void
OSystem::initBuffer (char *buffer, int width, int height)
{
  sdl_buffer =
    SDL_CreateRGBSurfaceFrom (buffer, width, height, 8, 640, 0, 0, 0, 0);
}

void
OSystem::crossFade (char *buffer, char *palette)
{
  ...
  backupSurface =
    SDL_CreateRGBSurface (SDL_SWSURFACE, 640, 480, 32, rmask, gmask, bmask,
			  0);
  newSurface =
    SDL_CreateRGBSurface (SDL_SWSURFACE | SDL_SRCALPHA, 640, 480, 32, rmask,
			  gmask, bmask, 0);
