  ...
  do
      ...
      if (temp == 20)
	{
	  if (enterPlayerName (42))
	    {
/*				do                // Cette boucle essai des noms de fichiers jusqu'a trouver un nom de fichier vide. Probleme si on a plus de 9999 sauvegarde dans le meme repertoire
				{
					saveGameFileName[0]=mainMenuVar3;
					saveGameFileName[1]=0; // pas dans le code original..
					// random pour avoir un num entre 0000 et 9999
					strcat(saveGameFileName,mainMenu7(10000*rand()));
					strcat(saveGameFileName,".LBA");
					saveGame=fileOpen(saveGameFileName);
				}while(!saveGame);*/
	      reinitAll (1);
	      newGame ();
	      if (mainLoop ())
...
void
LBA_engine::mainMenu2 (void)
{
//todo: implement this
}

int
LBA_engine::choosePlayerName (int param)
{
  return (0);
}

char *
LBA_engine::printString (int a, char *b)
{
  if (a == mainMenu3Var1)
    {
...
//      *(currentTextPtr+currentTextLength)=0;                    // on ajoute un \n a la fin
  ...
  copyStringToString (b, mainMenu3Var3, currentTextLength);	// enfin, on copie le tout dan mainMenu3Var3
...
int
LBA_engine::processMenu (short int *menuData)
{
  int localTime;
  int numEntry;
...
void
LBA_engine::drawSelectableLetters (void)
{
  char x, y;
...
void
LBA_engine::drawSelectableLetter (int x, int y, int arg)
{
  char buffer[256];
//      char param;
  ...
  if (arg != 0)
    ...
    {
      drawBoxTrans (left, top, right, bottom, (char *) videoBuffer2, left,
		    top, (char *) videoBuffer1);
      right2 = right;
  ...
  printStringSimple (centerX - getStringLength (buffer) / 2, centerY - 18,
		     buffer);
...
int
LBA_engine::enterPlayerName (short int param)
{
  char buffer[256];
...
char *
LBA_engine::getStringFromNum (int valeur)
{
//toto: implement this
  char *text;
...
void
LBA_engine::drawButton (short int *data, int a)
{
  int buttonNumber;
  int maxButton;
//      int var4;
...
void
LBA_engine::drawButtonGFX (int largeur, int posY, int c, int d, int mode)
{
...
void
LBA_engine::drawBoxTrans (int left, int top, int right, int bottom, char *source, int leftDest, int topDest, char *dest)	// pout les boutons actifs
{
...
void
LBA_engine::drawBoxInsideTrans (int left, int top, int right, int bottom,
				int mode)
{
...
void
LBA_engine::drawBoxOutLine (int left, int top, int right, int bottom)
{
...
void
LBA_engine::launchStringProcess (int a, int b, int c, int d, int e)
{
  drawLine (a, b, c, d, e);
}
...
// d=y2;
void
LBA_engine::drawLine (int a, int b, int c, int d, int e)
{
  int temp;
  short int flag;
  int flag2;
...
int
LBA_engine::getStringLength (char *string)
{
  stringLenght = 0;
  ...
  do
      ...
      else
	{
	  ...
	  stringLenght +=
	    *(fntFont + *((short int *) (fntFont + caractere * 4)));
	}
    }
  while (1);
...
void
LBA_engine::copyStringToString (char *a, char *b, int c)
{
  int i;
