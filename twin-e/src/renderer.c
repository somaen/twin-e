...
int LBA_renderer::startRenderer (int X, int Y, int Z, int angleX,int angleY, int angleZ, unsigned char *costumePtr)
{
  unsigned char *ptr;
...
void
LBA_renderer::setSomething4 (int angleX, int angleY, int angleZ)
{
	...
	if(setSomethingVar4 !=0) // use standard projection
	{
		do
		{
			...
			coZ = -(pointPtr->z + _Z);
 			...
 			pointPtrDest->x=(((coX + coZ) * 8 + (coX + coZ) * 16) >> 9) + setSomethingVar1;
			pointPtrDest->z = coZ - coX - coY;
	  		pointPtrDest->y =((((coX - coZ) * 4 + (coX - coZ) * 8) +
	      (2 * coY - (coY << 5))) >> 9) + setSomethingVar2;

			if(pointPtrDest->x < _renderLeft) _renderLeft=pointPtrDest->x;
			if(pointPtrDest->x > _renderRight) _renderRight=pointPtrDest->x;
		
			if(pointPtrDest->y < _renderTop) _renderTop=pointPtrDest->y;
			if(pointPtrDest->y > _renderBottom) _renderBottom=pointPtrDest->y;
...
// int* ptr1;
  ...
  //renderV19= dest
  //ebp= source
  renderS1S1 ((int *) _currentMatrixTableEntry, ebp);	// copie dans renderTab2 + application de la rotation

  // ? , numOfPoint , destination, rotation data
  renderS1S2 (_pointsPtr + rs1v1, rs1v2, &_projectedPointTable[rs1v1 / 6], (int *) _currentMatrixTableEntry);	//rotation des elements du model
}

void
LBA_renderer::renderS1S2 (unsigned char *esi, int ecx, pointTab * dest,
			int *eax)
{
  short int param1;
  short int param2;
  ...
  do
    {
      ...
      dest->x =
	((eax[0] * param1 + eax[1] * param2 + eax[2] * param3) >> 14) + _destX;
      dest->y =
	((eax[3] * param1 + eax[4] * param2 + eax[5] * param3) >> 14) + _destY;
      dest->z =
	((eax[6] * param1 + eax[7] * param2 + eax[8] * param3) >> 14) + _destZ;
...
void
LBA_renderer::renderS1S1 (int *eax, int *ebp)
{
  int angle;
  int angleVar1;		//esi
  int angleVar2;		//ecx
...
void
LBA_renderer::renderS2 (int edx, int ecx, int ebx, pointEntry * esi)
{
  int *dest;
  int *source;
...
void
LBA_renderer::renderS2Sub (unsigned char *esi, int ecx, pointTab * dest,
			 int *eax)
{
  short int param1;
  short int param2;
...
int
LBA_renderer::finishRender (unsigned char *esi)
{
  unsigned char *edi;
  short int temp;
  int eax, ecx;
...
void
LBA_renderer::polyRender (int ecx, int edi)
{
...
//  osystem->drawBufferToScreen(videoBuffer1);
  ...
  switch (ecx)
    ...
    case 0:			// flat polygone
      {
	eax =
	  (color & 0xFF) | ((color & 0xFF) << 8) | ((color & 0xFF) << 16) |
	  ((color & 0xFF) << 24);
	for (i = 0; i < vsize; i++)
	  {
	    stop = ptr1[480];
...
int
LBA_renderer::prepareRender (void)
{
