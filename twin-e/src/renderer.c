...
int
    ...
    LBA_renderer::startRenderer(int X, int Y, int Z, int angleX, int angleY,
				int angleZ, unsigned char *costumePtr)
{
    unsigned char *ptr;
...
void LBA_renderer::renderS2(int edx, int ecx, int ebx, pointEntry * esi)
{
    ...
    renderS2Sub(_pointsPtr + esi->data1, esi->data2,
		&_projectedPointTable[esi->data1 / 6], (int *) _currentMatrixTableEntry);
}
   ...
   // short int borrom;
	...
	do			// loop that load all the polygones
	{
		...
		do {
		    ...
		    shadeValue =
			currentPolyHeader->colorIndex + shadeTable[currentPolyVertex->shadeEntry];
