...
int
LBA_engine::loadImageToPtr (char *resourceName, byte * ptr, int imageNumber)	// en fait, c'est pas vraiment une image, ca peut etre n'importe quelle data...
{
  FILE *resourceFile;
  int headerSize;
  int offToImage;
...
void
LBA_engine::copyToBuffer (byte * source, byte * destination)
{
...
void
LBA_engine::fadeIn (byte * palette)
{
  int i;
...
void
LBA_engine::adjustPalette (byte R, byte G, byte B, byte * palette,
			   int intensity)
{
...
int
LBA_engine::remapComposante (int modifier, int color, int param,
			     int intensity)
{
  if (!param)
    return (color);
...
byte *
LBA_engine::loadImageToMemory (char *fileName, short int imageNumber)
{
  FILE *resourceFile;
  int headerSize;
  int offToImage;
...
void
LBA_engine::loadImageAndPalette (int imageNumber)
{
...
void
LBA_engine::loadImageCrossFade (int imageNumber)
{
  loadImageToPtr ("ress.hqr", videoBuffer2, imageNumber);
  copyToBuffer (videoBuffer2, videoBuffer1);
  loadImageToPtr ("ress.hqr", palette, imageNumber + 1);
...
void
LBA_engine::fadeOut (char *palette)
{
  int i;
...
void
LBA_engine::fadeIn2 (char *palette)
{
  int i;
  for (i = 0; i <= 100; i += 3)
    {
...
void
LBA_engine::resetVideoBuffer1 (void)
{
/*	int i;
...
void
LBA_engine::convertPalToRGBA (byte * palSource, byte * palDest)
{
  int i;
